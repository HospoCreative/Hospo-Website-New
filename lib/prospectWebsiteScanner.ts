...
export async function runProspectWebsiteScan(input: {
  websiteUrl: string;
  businessType: string;
  onPageCollected?: (page: { evidence: Evidence[]; pagesDiscovered: number; pagesScanned: number }) => Promise<void>;
}) {
  ...
  const incrementalEvidence = results.flatMap((result) => result.evidence);
  return { finalUrl: primary.toString(), pagesDiscovered: queue.length, pagesScanned: results.length, evidence: uniqueEvidence, incrementalEvidence, failures };
}
