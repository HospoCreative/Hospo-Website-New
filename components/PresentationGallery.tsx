...
export function PresentationGallery({ locale, id }: { locale: Locale; id?: string }) {
  ...
  return (
    <section id={id} className="overflow-hidden bg-ink px-5 py-[var(--hc-section)] text-white sm:px-8">
      <div className="mx-auto max-w-7xl">
        <Reveal>
          <div className="max-w-5xl">
            <p className="section-eyebrow text-yellow">{sectionCopy.eyebrow}</p>
            <h2 className="mt-5 font-serif text-[clamp(2.5rem,5.4vw,5rem)] font-semibold leading-[0.96]">
              {sectionCopy.title}
            </h2>
            <p className="mt-6 max-w-4xl text-lg leading-8 text-white/72 sm:text-xl sm:leading-9">
              {sectionCopy.body}
            </p>
            <Link href={localizedPath("/services/photography-video", locale)} className="mt-7 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.15em] text-white transition hover:text-yellow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow">
