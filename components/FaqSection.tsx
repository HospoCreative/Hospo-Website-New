  ...
  return (
    <section className="bg-ink px-5 py-[var(--hc-section-compact)] text-white sm:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.65fr_1.35fr] lg:gap-16">
        <SectionHeading tone="light" eyebrow={content.eyebrow} title={content.title} />
        <div className="border-t border-white/18">
          {content.items.map((item, index) => {
            const open = openIndex === index;
            const panelId = `faq-panel-${index}`;
            return (
              <div key={item.question} className="border-b border-white/18">
                ...
                <div id={panelId} hidden={!open} className="pb-5 pr-10 text-base leading-7 text-white/70">{item.answer}</div>
              </div>
            );
          })}
        </div>
