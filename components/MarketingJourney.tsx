...
export function MarketingJourney() {
  ...
  return (
    <section id="guest-journey" className="bg-white px-5 py-[var(--hc-section)] text-ink sm:px-8">
      <div className="mx-auto max-w-7xl">
        <SectionHeading eyebrow={content.eyebrow} title={content.title} body={content.body} width="wide" />
        <div className="relative mt-12">
          <div className="absolute bottom-0 left-[7px] top-0 w-px bg-ink/18 md:bottom-auto md:left-0 md:right-0 md:top-[7px] md:h-px md:w-auto" aria-hidden="true" />
          <motion.div
          ...
          <ol className="grid gap-7 pl-8 md:grid-cols-5 md:gap-5 md:pl-0 md:pt-0">
            {content.stages.map((stage, index) => (
              <motion.li key={stage.title} initial={reduceMotion ? false : { opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.45, delay: index * 0.08 }} className="relative md:pt-8">
                <span className="absolute -left-[1.95rem] top-1 size-3.5 rounded-full border-2 border-yellow bg-white md:left-0 md:top-0" aria-hidden="true" />
                <h3 className="font-serif text-2xl font-semibold">{stage.title}</h3>
                <p className="mt-3 text-[0.95rem] leading-6 text-ink/68">{stage.body}</p>
              </motion.li>
            ))}
          </ol>
        </div>
